...
class RolloutPolicyNet(nn.Module):
    def __init__(self, n_rules, fp_dim=2048, dim=512,
        ...
        self.fc1 = nn.Linear(fp_dim,dim)    #神经网络线性层，输入为2048，中间层为512
        self.bn1 = nn.BatchNorm1d(dim)      #加速神经网络训练，将每个batch的数据规范化为统一的分布
        self.dropout1 = nn.Dropout(dropout_rate)    #以0.3的概率将输入的张量元素随机归零，常用的正则化器，用于防止网络过拟合
        # self.fc2 = nn.Linear(dim,dim)
        # self.bn2 = nn.BatchNorm1d(dim)
        # self.dropout2 = nn.Dropout(dropout_rate)
        self.fc3 = nn.Linear(dim,n_rules)   #隐藏层为512，输出为规则数量

    def forward(self,x, y=None, loss_fn =nn.CrossEntropyLoss()):
        x = self.dropout1(F.elu(self.bn1(self.fc1(x))))     #elu激活函数，与ReLU不同的是，ELU没有神经元死亡的问题，已被证明优于ReLU及其变体
...
def load_parallel_model(state_path, template_rule_path,fp_dim=2048):    #state_path:saved_rollout_state_1_2048.ckpt; template_rule_path:template_rules_1.dat
    template_rules = {}
    with open(template_rule_path, 'r') as f:
        for i, l in tqdm(enumerate(f), desc='template rules'):  #tdqm显示进度条，desc是进度条的前缀；enumerate(f)遍历f中的数据。
            rule= l.strip()                                     #strip() 方法用于移除字符串头尾指定的字符（默认为空格或换行符）或字符序列
            template_rules[rule] = i
    idx2rule = {}                               #储存化学反应转化规则模板
    for rule, idx in template_rules.items():    #将template_rules中的字符串对应索引转化为索引对应字符串
        idx2rule[idx] = rule
    rollout = RolloutPolicyNet(len(template_rules),fp_dim=fp_dim)
