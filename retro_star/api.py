...
class RSPlanner:
    def __init__(self,
        ...
        self.plan_handle = prepare_molstar_planner(
            ...
            expansion_topk=expansion_topk,  #50
    ...
    def plan(self, target_mol):
        ...
        if succ:
            result = {
                ...
                'best_routes': msg[2],
                'predict_reaction_smiles': msg[2].pop(0),
...
if __name__ == '__main__':
    ...
    result = planner.plan('O=C1C2([H])C=C(CO)[C@@H](O)[C@]3(O)[C@@H](O)C(C)=C[C@@]31[C@H](C)C[C@]4([H])[C@@]2([H])C4(C)C')
    print(result)
